import * as Yup from 'yup';const SUPPORTED_FORMATS = ['image/jpg', 'image/jpeg', 'image/png', 'image/bmp', 'image/gif', 'image/svg', 'image/webp'];
...
export const PostSchema = Yup.object().shape(
	{
		...
		images: Yup.mixed()
			.nullable()
			.when('content', {
				is: (content: string) => content?.length <= 0,
