...
export const SideItem: React.FC<SideItemProps> = ({ title, link, icon, target = '_self' }) => {
	return (
		<Link href={link}>
			<a target={target} title={title} rel={target !== '_self' ? 'noreferrer noopener' : ''} className="h-12 flex items-center gap-2 hover:bg-dark-100 transition-colors rounded-md p-1">
